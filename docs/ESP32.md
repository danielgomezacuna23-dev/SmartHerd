...
El proyecto Supabase está configurado. El 25/9/2026 se actualizaron ambos módulos, uno por uno, comprobando sus series USB e ID internos antes de grabarlos. Con ambos encendidos se confirmó el cambio de 300 a 5 segundos, dos tramas `NO_FIX` recibidas con cinco segundos de separación y el retorno a 300 segundos. También se confirmó el control mediante `POST /mode` del puente local. Faltan la conexión autónoma de la estación con Supabase y una posición GPS real bajo cielo abierto.
