...
export function makeSeed() {
  const animals = ["Estrella", "Canela", "Lucero", "Paloma", "Mora"].map(
    (name, i) => ({
      id: uid(),
      name,
      ear_tag: `CR-0884${i + 1}`,
      breed: ["Holstein", "Brahman", "Brangus", "Jersey", "Pardo suizo"][i],
      purpose: ["leche", "doble", "engorde", "doble", "leche"][i],
      sex: i === 2 ? "macho" : "hembra",
      birth_date: `202${i === 4 ? 5 : 2}-03-14`,
      status: "activo",
    }),
  );
  const devices = animals
    .slice(0, 4)
    .map((a, i) => ({
      id: `SH-COLLAR-00${i + 1}`,
      animal_id: a.id,
  ...
  const readings = devices.flatMap((d, i) =>
    Array.from({ length: 12 }, (_, j) => ({
      ...
      recorded_at: new Date(
        Date.now() - (11 - j) * 3600000 - (i === 2 ? 7200000 : 0),
      ).toISOString(),
      temperature_c:
        Math.round(
          (33 + Math.sin(j) * 0.5 + (i === 1 && j === 11 ? 3 : 0)) * 10,
        ) / 10,
      activity: i === 1 && j === 11 ? 86 : 25 + (j % 5),
      battery_pct: 82 - i * 20,
      latitude: 10.001 + i * 0.0006,
      longitude: i === 3 ? -84.108 : -84.116 + i * 0.0007,
    })),
  );
  const events = [
    {
